...
def register_routes(app):
    ...
    def post_user_message(report_id):
        ...
        # Optionally emit a socket event to notify about the new comment
    ...
    def admin():
        ...
        faculties = [f[0] for f in db.session.query(Report.faculty).distinct().order_by(Report.faculty).all()]
        selected_faculty = request.args.get('faculty') or (faculties[0] if faculties else None)
    ...
    def submit_suggestion():
        ...
        new_suggestion = Suggestion(suggestion=suggestion_text, faculty=faculty, reason=reason, user=current_user)
        try:
            db.session.add(new_suggestion)
            db.session.commit()
        except Exception:
            db.session.rollback()
            flash('Error submitting suggestion.', 'danger')
    ...
    def upvote(suggestion_id):
        ...
        existing_vote = Vote.query.filter_by(user_id=current_user.id, suggestion_id=suggestion.id).first()
        if existing_vote:
            flash("You have already upvoted this suggestion.", 'info')
        else:
            vote = Vote(user=current_user, suggestion=suggestion)
            suggestion.votes += 1
            db.session.add(vote)
            db.session.commit()
